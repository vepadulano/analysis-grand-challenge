...
def construct_fileset(ntuples_file: str,
                      n_files_max_per_sample: int,
                      storage_location: str,
                      use_merged_dataset: bool = True):
    ...
    for process in file_info.keys():
        ...
        for variation in file_info[process].keys():
            ...
            if (storage_location == "cern-xrootd"):
                ...
                raise RuntimeError("Benchmark can only run on 'cern-xrootd' dataset at this moment.")
...
def save_histograms(all_histograms, fileset, filename):
    ...
    pseudo_data = (all_histograms[:, :, "ttbar", "ME_var"] + all_histograms[:, :,
                   "ttbar", "PS_var"]) / 2 + all_histograms[:, :, "wjets", "nominal"]
    ...
    with uproot.recreate(filename) as f:
        for region in ["4j1b", "4j2b"]:
            ...
            for sample in nominal_samples:
                ...
                for i in range(4):
                    for direction in ["up", "down"]:
                        ...
                        f[f"{region}_{sample_name}_{variation_name}"] = all_histograms[120j::hist.rebin(
                            2), region, sample_name, variation_name]
            ...
            # W+jets scale
            f[f"{region}_wjets_scale_var_down"] = all_histograms[120j:: hist.rebin(
                2), region, "wjets", "scale_var_down"]
