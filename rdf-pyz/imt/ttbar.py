...
PARSER.add_argument("--storage-location", "-l",
                    help="Where the data resides. Default downloads original dataset stored at UNL.", default="unl")
...
class TtbarAnalysis(dict):
    ...
    def fill(self, process, variation):
        ...
        lumi = 3378  # /pb
        xsec_weight: float = x_sec * lumi / nevts_total
        d = d.Define('weights', lambda: xsec_weight)  # default weights
        ...
        for region in ["4j1b", "4j2b"]:
            ...
            if region == "4j1b":
                ...
                fork = d.Filter(lambda jet_btag, jet_pt_mask: numpy.sum(jet_btag[jet_pt_mask] >= 0.5) == 1)\
                        .Define(observable, lambda jet_pt, jet_pt_mask: numpy.sum(jet_pt[jet_pt_mask]))
